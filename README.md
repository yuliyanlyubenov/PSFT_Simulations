...
| **32_self_gravitating_geon_geodesics.py** | paper Section 7.4 follow-up | **First null-geodesic ray-tracing on a NUMERICALLY-EVOLVED metric.** Builds a static metric via the Lichnerowicz solver (compact Gaussian matter source), wraps it in a `BSSNGriddedMetric` with trilinear interpolation, then traces null geodesics through it via the existing `GeodesicEvolver`. At moderate compactness the metric induces clear gravitational focusing: photons launched with small impact parameter pass through the high-curvature core (chi depression), those with large b stay in the periphery. Null condition `|u·u|` preserved to 1e-5 over the entire evolution. Demonstrates that the curved-background extension produces TRAPPING-PRECURSOR geometry from MATTER, not analytic forms. Full strong-field trapping (the discontinuous-at-Kc^strong transition predicted by paper Section 7.4) is the natural follow-on requiring full-Valencia BSSN. |
| **33_dynamical_radiation_outflow.py** | paper Section 7.4 follow-up | **First time-resolved matter→light release demonstration.** Initial: static Coulomb field of a Gaussian +Q source. At t=0 the source is REMOVED and the photonic field evolves freely; the field that was previously sourced now propagates outward at the wave speed. Interior U_EM (within r=0.25L of the source) drops by **96.3%** as radiation exits; outflow-region U_EM shows the expected transient peak. Critically, the SCALAR-WAVE energy `E_wave = (1/2)∫(π_t² + |∇A_t|²)d³x` (the proper conserved quantity for the freely-evolving wave equation after the source is removed and Lorenz gauge is dynamically violated) is preserved to **0.71%** over 100 RK4 steps. This is the time-resolved counterpart of Example 31's static topological-cancellation energy accounting -- annihilation as radiation outflow. |
| **34_literal_kretschmann_demonstration.py** | paper Section 7.4 follow-up | **First demonstration of the literal Kretschmann scalar replacing Example 20's `|∇Φ|²` proxy.** Solves the Lichnerowicz constraint for a Gaussian matter source, computes `K = R_abcd R^abcd` via `kretschmann_from_adm()` on the resulting BSSN state, and compares against the matter-gradient proxy. The literal K is finite, positive, peaks at the matter core, and correlates positively (Pearson ~ 0.8) with `|∇ρ|²`. Heaviside `Θ(K - Kc)` activation regions overlap on the high-curvature core. Unblocks the literal-K version of the Example 20 flux-tube simulation: once the matter+geometry coupled BSSN evolution is in place, the `|∇Φ|²` proxy retires and the literal `R_abcd R^abcd` enters the Postulate 3 Heaviside trigger. |
